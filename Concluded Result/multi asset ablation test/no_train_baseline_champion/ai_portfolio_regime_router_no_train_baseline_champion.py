...
class AIPortfolioRegimeRouter:
    ...
    def _fallback_pair_selection_score(self, row: dict[str, Any]) -> float:
        ...
        dd_consistency = float(row.get("drawdown_consistency_score", -1.0))
        max_excess = float(row.get("max_drawdown_excess_over_target", 0.0) or 0.0)
        return float(
            sharpe
            + 0.60 * dd_consistency
            - 3.50 * drawdown
    ...
    def _fallback_pair_selection(self, active_pair: str, candidate_pairs: list[dict[str, Any]]) -> dict[str, Any]:
        if not candidate_pairs:
            return {
                "action": "hold",
                "pair": active_pair,
                "reason": "AI selection unavailable and no fallback candidates",
                "confidence": 0.0,
            }
        scored = [
            (self._fallback_pair_selection_score(row), str(row.get("pair")), row)
        ...
        return {
            "action": "switch",
            "pair": scored[0][1],
            "reason": "AI selection fallback: best Sharpe/drawdown/consistency/CVaR score",
            "confidence": 0.0,
        }
    ...
    def switch_review(
        ...
        try:
            if not self.ai_enabled:
                raise ValueError("AI disabled")
            decision = self._call_ai(json.dumps(payload, default=str), AI_PORTFOLIO_SWITCH_PROMPT)
    ...
    def choose_pair(
        ...
        try:
            ...
            decision = self._fallback_pair_selection(active_pair, candidate_pairs)
