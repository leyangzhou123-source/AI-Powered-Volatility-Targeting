...
def _ann_vol(returns: pd.Series | np.ndarray, ann: float = 252.0) -> float:
    r = pd.Series(returns).astype(float).replace([np.inf, -np.inf], np.nan).dropna()
    if len(r) < 2:
        return 0.0
    return float(r.std(ddof=1) * math.sqrt(float(ann)))


def _pair_train_metrics(pair_df: pd.DataFrame, target_vol: float, ann: float = 252.0) -> dict[str, float]:
    ...
    valid = valid & (vol > 0)
    if int(valid.sum()) >= 5:
        rv = np.sqrt(np.maximum(float(ann) * asset[valid].to_numpy() ** 2, 1e-12))
    ...
    realized_vol = _ann_vol(r, ann=ann)
    return {
        ...
        "cost_adjusted_sharpe": _rolling_sharpe(r.to_numpy(), ann=ann),
...
def _benchmark_metrics(
    ...
    turnover = weight.diff().abs().replace([np.inf, -np.inf], np.nan).fillna(0.0)
    realized_vol = _ann_vol(returns, ann=ann)
    return {
        "name": "rv22_naive_scaling",
        "obs": int(len(returns)),
        "rolling_sharpe": _rolling_sharpe(returns.to_numpy(), ann=ann),
    ...
    for pair_name, pair_df in pair_results.items():
        ...
        for regime in ("low", "middle", "high"):
            ...
            out[regime][pair_name] = {
                ...
                "sharpe": _rolling_sharpe(returns.to_numpy(), ann=ann),
...
def _pair_window_metric(pair_df: pd.DataFrame, target_vol: float, ann: float = 252.0) -> dict[str, float]:
    if pair_df.empty:
        return {}
    ...
    return {
        ...
        "annualized_return": float(returns.mean() * float(ann)) if len(returns) else 0.0,
...
def _recent_pair_rankings(
    ...
    for window in windows:
        ...
        for pair_name, pair_df in pair_results.items():
            ...
            metrics = _pair_window_metric(hist, target_vol=target_vol, ann=ann)
...
def evaluate_precomputed_router_protocol(
    ...
    ann_factor = float(cfg.get("vol_ann", cfg.get("ann_factor", 252.0)))
    ...
    while start < len(market_df):
        ...
        for pair in pairs:
            ...
            metrics = _pair_train_metrics(train_df, target_vol=target_vol, ann=ann_factor)
        ...
        for j in range(n):
            ...
            for pair in pairs:
                ...
                if len(returns) >= 2 and not freeze_oos_metrics:
                    pair_perf[pair.name] = {
                        "obs": int(len(returns)),
                        "rolling_sharpe": _rolling_sharpe(returns, ann=ann_factor),
                        "drawdown": _max_drawdown(hist_equity),
                        "realized_vol": _ann_vol(returns, ann=ann_factor),
                        "vol_tracking_error": vol_track,
                        "turnover": float(np.mean(h["turnover"])) if len(h["turnover"]) else 0.0,
            ...
            obs = strategy_ret[:j]
            realized_vol = _ann_vol(obs, ann=ann_factor) if len(obs) >= 2 else 0.0
            ...
            perf = {
                "obs": int(j),
                "rolling_sharpe": _rolling_sharpe(obs, ann=ann_factor),
            ...
            pair_hist[pair.name]["vol_track"].append(
                abs(_ann_vol(strategy_ret[: j + 1], ann=ann_factor) - target_vol)
                if j >= 2
                else 0.0
            )
        ...
        market_vol = (
            pd.Series(market_df["returns_clean"].iloc[start:test_end].to_numpy())
            .rolling(21)
            .std()
            * math.sqrt(float(ann_factor))
        )
        ...
        router_rows.append(
            {
                ...
                "sharpe": _rolling_sharpe(strategy_ret, ann=ann_factor),
                "calmar": _calmar(strategy_ret, ann=ann_factor),
                "cvar_95": _cvar_95(strategy_ret),
                "bh_sharpe": _rolling_sharpe(bh_ret, ann=ann_factor),
                "bh_calmar": _calmar(bh_ret, ann=ann_factor),
                "bh_cvar_95": _cvar_95(bh_ret),
                "excess_sharpe": _rolling_sharpe(strategy_ret, ann=ann_factor)
                - _rolling_sharpe(bh_ret, ann=ann_factor),
                "regime_sharpe_high_vol": _rolling_sharpe(
                    rr[high_mask.fillna(False)].to_numpy(),
                    ann=ann_factor,
                )
                if high_mask.any()
                else np.nan,
                "regime_sharpe_low_vol": _rolling_sharpe(
                    rr[~high_mask.fillna(False)].to_numpy(),
                    ann=ann_factor,
                )
                if (~high_mask.fillna(False)).any()
                else np.nan,
    ...
    summary = pd.DataFrame(
        [
            {
                "overall_sharpe": _rolling_sharpe(oos, ann=ann_factor),
                "overall_calmar": _calmar(oos, ann=ann_factor),
                "overall_cvar_95": _cvar_95(oos),
                "overall_ann_return": float(np.mean(oos) * ann_factor),
                "overall_ann_vol": _ann_vol(oos, ann=ann_factor),
                "overall_drawdown": _max_drawdown(np.exp(np.cumsum(oos))),
                "bh_overall_sharpe": _rolling_sharpe(oos_bh, ann=ann_factor),
                "bh_overall_calmar": _calmar(oos_bh, ann=ann_factor),
                "bh_overall_cvar_95": _cvar_95(oos_bh),
                "bh_ann_return": float(np.mean(oos_bh) * ann_factor),
                "bh_ann_vol": _ann_vol(oos_bh, ann=ann_factor),
                "bh_drawdown": _max_drawdown(np.exp(np.cumsum(oos_bh))),
                "excess_sharpe": _rolling_sharpe(oos, ann=ann_factor)
                - _rolling_sharpe(oos_bh, ann=ann_factor),
